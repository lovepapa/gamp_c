	...
	const prcopt_t popt, const solopt_t sopt, filopt_t *fopt)
{
	...
	if (popt.navsys&SYS_CMP||popt.navsys&SYS_GAL||(strlen(fopt->p1c1dcbf)==0)) findMGEXDcbFile(ts, te, dir, fopt->mgexdcbf); //如果处理系统中包含BDS和GAL则用MGEX文件（BSX后缀)
