...
static int readdcbf(const char *file, nav_t *nav)
{
	...
	while (fgets(buff,sizeof(buff),fp)) {
		if      (strstr(buff,"DIFFERENTIAL (P1-P2) CODE BIASES")|| strstr(buff,"DIFFERENTIAL CODE BIASES SATELLITE (P1-P2)")) type=1;

		else if (strstr(buff,"DIFFERENTIAL (P1-C1) CODE BIASES")) type=2;
		else if (strstr(buff,"DIFFERENTIAL (P2-C2) CODE BIASES")) type=3;

		if (!type) continue;
        // note dcb文件在2016年以前DCB值在文件中存储的位数有所不同

		//if (!(sat=satid2no(buff))||(cbias=str2num(buff,26,9))==0.0) continue;
        if (!(sat = satid2no(buff)) || (cbias = str2num(buff, 8, 7)) == 0.0) continue;
                   ...
                   double *dts, double *vare, double *varc)
{
    ...
    /* polynomial interpolation for orbit 轨道的多项式插值  */
    i=index-(NMAX+1)/2;
    if (i<0) i=0; 
