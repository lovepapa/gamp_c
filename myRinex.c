...
//read time tag of the first epoch 读取首个历元的时间标签
...
//read time tag of the last epoch 读取最后一个历元的时间标签
...
extern int findSnxFile(const gtime_t ts, const gtime_t te, char dir[], char snxpaths[])
{
	...
	//此处搜索igs 或者 gfz 对应的 snx 产品。
	sprintf(tmp,"%s%cigs%4d.snx",dir,sep,weeks);
	//sprintf(tmp, "%s%cgfz%4d.snx", dir, sep, weeks);
	//sprintf(tmp, ("%s%cigs%4d.snx" || "%s%cgfz%4d.snx"), dir, sep, weeks);
