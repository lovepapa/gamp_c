...
//pseudorange observation checking 伪距观测值检查
extern void obsScan_SPP(const prcopt_t *popt, obsd_t *obs, const int nobs, int *nValid)
{
	...
	if (nValid) *nValid=n;  // 得到有效的SPP伪距观测值
}
extern void obsScan_PPP(const prcopt_t *popt, obsd_t *obs, const int nobs, int *nValid)
{
	...
	for (i=n=0;i<nobs&&i<MAXOBS;i++) {
		...
		if (fabs(obs[i].P[0]-obs[i].P[f2])>=200.0) continue; // |P1 - P2| > 200 m

		obs[n]=obs[i];
		n++;
	}

	if (nValid) *nValid=n;  // 得到有效的PPP伪距观测值
}
...
/* detect cycle slip by widelane jump 通过宽巷跳跃检测周跳 -----------*/
