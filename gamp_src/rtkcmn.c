...
const filopt_t filopt_default={ /* defaults solution output options */
	"","","","","","","","","","","",
	NULL,NULL,
	"","","",""
};
const char outtype[MAXOUTFILE][100]={
	{"pos\0     "}, {"debug\0   "}, {"pdop\0     "}, {"elev\0    "}, {"dtrp\0    "}, 
...
/* inner product ---------------------------------------------------------------
* inner product of vectors 向量的数量积
...
* outer product of 3d vectors 向量的叉乘
                    ...
                    const double *Qb, int n, double *xs, double *Qs)
{
    ...
    if (!matinv(invQf,n)&&!matinv(invQb,n)) {
        ...
        if (!(info=matinv(Qs,n))) {
            matmul("NN",n,1,n,1.0,invQf,xf,0.0,xx);/* xx=invQf*xf */
            matmul("NN",n,1,n,1.0,invQb,xb,1.0,xx);/* xx=invQf*xf + invQb*xb */
            matmul("NN",n,1,n,1.0,Qs,xx,0.0,xs);   /* xs=Qs*(Qf^-1*xf+Qb^-1*xb) */
        }
    }
...
*          double *pos      O   geodetic position {lat,lon,h}{B,L,H} (rad,m)
* return : none
...
/* transform covariance to local tangental coordinate 将协方差转换为局部切线坐标----
...
extern void covenu(const double *pos, const double *P, double *Q)
{
    ...
    xyz2enu(pos,E);
	matmul("NN", 3, 3, 3, 1.0, E, P, 0.0, EP); /* EP =  E*P */
    matmul("NT",3,3,3,1.0,EP,E,0.0,Q);         /*  Q = EP*E' */
}
...
/* astronomical arguments: f={l,l',F,D,OMG} (rad) // 天文系数 ---------------------*/
static void ast_args(double t, double *f)
{
    static const double fc[][5]={ /* coefficients for iau 1980 nutation //iau 1980章动的系数 */
...
/* eci to ecef transformation matrix //eci到ecef转换矩阵--------------------------
...
/* decode antenna parameter field //解码天线参数字段-----------------------------*/
...
static void addpcv(const pcv_t *pcv, pcvs_t *pcvs)
{
    ...
    if (pcvs->nmax<=pcvs->n) {
        ...
        if (!(pcvs_pcv=(pcv_t *)realloc(pcvs->pcv,sizeof(pcv_t)*pcvs->nmax))) {
            sprintf(PPP_Glo.chMsg,"*** ERROR: addpcv: memory allocation error\n");
...
extern int readblq(const char *file, const char *sta, double *odisp)
{
    ...
    if (!(fp=fopen(file,"r"))) {
		sprintf(PPP_Glo.chMsg,"*** ERROR: blq file open error: file=%s\n",file);
        outDebug(OUTWIN,OUTFIL,0);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
    ...
    sprintf(PPP_Glo.chMsg,"*** WARNING: no otl parameters: sta=%s file=%s\n",sta,file);
	outDebug(OUTWIN,OUTFIL,0);
...
extern int readerp(const char *file, erp_t *erp)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        erp->data[erp->n].xp=v[1]*1E-6*AS2R;  /* 单位统一为m */
...
/* sort and unique ephemeris //排序和独特的星历-------------------------------------*/
static void uniqeph(nav_t *nav)
{
    ...
    if (!(nav_eph=(eph_t *)realloc(nav->eph,sizeof(eph_t)*nav->n))) {
        printf("uniqeph malloc error n=%d\n",nav->n);
...
static void uniqgeph(nav_t *nav)
{
    ...
    if (!(nav_geph=(geph_t *)realloc(nav->geph,sizeof(geph_t)*nav->ng))) {
        printf("uniqgeph malloc error ng=%d\n",nav->ng);
...
extern int sortobs(obs_t *obs)
{
	...
	/* delete duplicated data 删除重复的数据*/
	for (i=j=0;i<obs->n;i++) {
		...
		//如果有重复历元，提示输出
			...
			for (k=0;k<NFREQ;k++) {
				...
				if (obs->data[i].P[k]!=obs->data[j].P[k]) {bDif=1; break;}
...
/* expand file path ------------------------------------------------------------
* expand file path with wild-card (*) in file //使用文件中的通配符（*）扩展文件路径
...
/* create directory ------------------------------------------------------------
* create directory if not exist  // 创建目录（如果不存在）
...
extern void dops(int ns, const double *azel, double elmin, double *dop)
{
    ...
    matmul("NT",4,4,n,1.0,H,H,0.0,Q); /* Q = H*H' */
    if (!matinv(Q,4)) {
...
/* interpolate antenna phase center variation // 插值天线相位中心变化-------------*/
                       ...
                       double *rmoon, double *gmst)
{
	...
	if (rsun) matmul("NN", 3, 1, 3, 1.0, U, rs, 0.0, rsun); /*  rsun = U*rs */
    if (rmoon) matmul("NN",3,1,3,1.0,U,rm,0.0,rmoon);       /* rmoon = U*rm */
    if (gmst ) *gmst=gmst_;
}
/* carrier smoothing -----------------------------------------------------------
