...
//      Surface pressure on the geoid   大地水准面上的表面压力                                            
		APM = 0.0;
		APA = 0.0;
		I = 0;
...
//      height correction for pressure  压力高度校正                                            
		PRES = PRES0*pow(1.0-0.0000226*HORT, 5.225);

//      Surface temperature on the geoid  大地水准面的表面温度                                          
		ATM = 0.0;                                                              
		ATA = 0.0;                                                                 
		I = 0;
...
//      height correction for temperature   高度校正温度
...
extern void tropmapf_gmf(gtime_t gt, const double blh[3], double elev, double *gmfh, double *gmfw)
{
	...
	if (dlat<0.0) {	//southern hemisphere  南半球
		phh=PI;
		c11h=0.007;
		c10h=0.002;
	}
	else {	//northern hemisphere  北半球
		phh=0;
		c11h=0.005;
		c10h=0.001;
	}
