...
/* exclude meas of eclipsing satellite (block IIA) 排除对星蚀卫星的影响---------*/
	...
	const double *rs, double *exs, double *eys)
{
	...
	/* satellite fixed x,y-vector 卫星固定x，y向量 */
	cross3(en,es,ex);
	cosy=cos(yaw);
	siny=sin(yaw);
	for (i=0;i<3;i++) {
	...
	const double *rs, const double *rr, double *phw)
{
	...
	/* satellite yaw attitude model 卫星偏航姿态模型 */
...
/* measurement error variance 测量误差方差 ------------------------------------*/
...
/* initialize state and covariance 初始化状态和协方差 ---------------------------*/
...
/* antenna corrected measurements 天线校正测量-------------------------------*/
...
/* temporal update of position 位置的时间更新 -------------------------------*/
static void udpos_ppp(rtk_t *rtk)
{
	...
	/* kinmatic mode without dynamics 没有动力学的运动模式 */
	for (i=0;i<3;i++) {
...
static void udbias_ppp(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	/* handle day-boundary clock jump 处理日界钟跳 */
	clk_jump=ROUND(time2gpst(obs[0].time,NULL)*10)%864000==0;

	ecef2pos(rtk->sol.rr,pos);

	for (f=0;f<NF(&rtk->opt);f++) {
		/* reset phase-bias if expire obs outage counter 如果超过了计数器，则重置相位偏差 */
		for (i=0;i<MAXSAT;i++) {
		...
		/* correct phase-code jump to ensure phase-code coherency  正确的相位代码跳转以确保相位代码一致性 */
		...
		for (i=0;i<n&&i<MAXOBS;i++) {
			...
			/* reinitialize phase-bias if detecting cycle slip 如果检测到周跳，则重新初始化相位偏差 */
			initx(rtk,bias[i],VAR_BIAS,IB(sat,f,&rtk->opt));

			/* reset fix flags 重置修复标志 */
...
/* satellite antenna phase center variation 卫星天线相位中心变化------------*/
	...
	double *v, double *H, double *R, double *azel)
{
	...
	for (i=0;i<n&&i<MAXOBS;i++) {
		...
		//Gravitational delay correction 获得广义相对论（红移）对GPS信号影响的重力延迟校正 */
		...
		for (j=0;j<2*NF(opt);j++) {
			...
			/* reject satellite by pre-fit residuals 通过先验拟合残差拒绝卫星 */
			...
			/* record large post-fit residuals 记录较大的验后拟合残差  */
		...
		//constraint to external ionosphere correction 对外部电离层校正的约束
	...
	sys=rtk->opt.navsys;
	//add datum constraint for GLOICB_1SAT  为GLOICB_1SAT添加基准约束
	...
	//add datum constraint for GLOICB_1FRQ  为GLOICB_1FRQ添加基准约束
...
/* number of estimated states 估计状态的数量 ---------------------------------*/
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
	...
	if (PPP_Glo.outFp[OFILE_IPPP]) {
		...
		//the displacements by earth tides 地球潮汐的位移
