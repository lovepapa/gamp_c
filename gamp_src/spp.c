...
/* pseudorange measurement error variance 伪距测量误差方差---------*/
...
/* psendorange with code bias correction 具有码偏差校正的伪距 ------------*/
				   ...
				   double *v, double *H, double *var, double *azel, int *vsat, double *resp, int *nx, int *bDeleted)
{
    ...
    *nx=NX_SPP;
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        /* reject duplicated observation data 拒绝重复的观察数据 */
        ...
        /* geometric distance/azimuth/elevation angle 几何距离/方位角/仰角 */
                  ...
                  char *msg, double dop[4])
{
    ...
    /* chi-square validation of residuals 残差的卡方验证 */
    vv=dot(v,v,nv);
    if (nv>nx) {
