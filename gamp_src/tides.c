...
/* solar/lunar tides (ref [2] 7) 太阳/月球潮汐---------------------------------*/
...
/* displacement by solid earth tide (ref [2] 7) 固体潮流引起的位移---------*/
                       ...
                       double *dr)
{
    ...
    /* eliminate permanent deformation 消除永久变形 */
    if (opt&8) {
        sinl=sin(pos[0]); 
...
/* displacement by ocean tide loading (ref [2] 7) 海潮负荷引起的位移---------*/
                     ...
                     const double *odisp, double *dr)
{
    ...
    if ((opt&2)&&odisp) { /* ocean tide loading */
		...
		matmul("TN", 3, 1, 3, 1.0, E, denu, 0.0, drt); /* drt = E'*denu */
        for (i=0;i<3;i++) dr[i]+=drt[i];
    }
