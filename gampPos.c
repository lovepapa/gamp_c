...
#define MAXPRCDAYS  1024        /* max days of continuous processing */
...
/* open output file for append 打开输出文件以进行追加--------------------*/
static FILE *openfile(const char *outfile)
{
    return !*outfile?stdout:fopen(outfile,"a");
}
//calculate sampling interval 计算采样间隔
...
static int clkRepair(obsd_t *obs, int n)
{
	...
	for (i=0;i<n;i++) {
		...
		if(fabs(d1-d3)>290000)   //ms clock jump（单位为ms）
		{
			delta0+=d1-d3;
			delta1+=d2-d4;
			cjGps++;
	...
	for (i=0;i<n;i++) 
	{
		...
		//repair for phase observations 仅对载波观测值进行了钟跳修复
...
//calculate DOPs {GDOP,PDOP,HDOP,VDOP} 返回PDOP值
...
static void procpos(rtk_t *rtk,const prcopt_t *popt,const solopt_t *sopt,int mode)
{
	...
	while ((nobs=inputobs(obs,obss,PPP_Glo.revs,&PPP_Glo.iObsu,&PPP_Glo.iEpoch))>=0) {
		...
		if (PPP_Glo.prcOpt_Ex.navSys&SYS_CMP) {
			BDmultipathCorr(rtk,obs,n);//  BDS的IGSO与MEO多路径改正
		}
...
static int execses(prcopt_t *popt, const solopt_t *sopt, filopt_t *fopt)
{
	...
	if (popt->soltype==0) {       /* forward */
		...
		PPP_Glo.solf=(sol_t *)malloc(sizeof(sol_t)*PPP_Glo.nEpoch); //malloc表示动态分配内存，后面一定要free释放
	               ...
	               const solopt_t *sopt, filopt_t *fopt)
{
	...
	if (*fopt->mgexdcbf&&(popt->navsys&SYS_CMP||popt->navsys&SYS_GAL||(strlen(fopt->p1p2dcbf)==0)))
